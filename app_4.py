    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            if "BB" not in file.filename:
                return render_template('banana_detection.html', alert="Please upload a valid banana image with 'BB' in the filename.")
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            if "MG" not in file.filename:
                return render_template('mango_detection.html', alert="Please upload a valid mango image with 'MG' in the filename.")
            img = Image.open(io.BytesIO(file.read())).convert("RGB")
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            if "PG" not in file.filename:
                return render_template('pomogranate_detection.html', alert="Please upload a valid pomegranate image with 'PG' in the filename.")
            img = Image.open(io.BytesIO(file.read())).convert("RGB")
